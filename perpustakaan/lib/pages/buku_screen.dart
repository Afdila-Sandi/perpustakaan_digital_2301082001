...
class _BukuScreenState extends State<BukuScreen> {
  ...
  Future<void> _getData() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final books = List<Map<String, dynamic>>.from(jsonDecode(response.body)['data']);
  ...
  void _showFormDialog({Map<String, dynamic>? book}) {
    ...
    final _pengarangController = TextEditingController(text: book?['pengarang'] ?? '');
    final _penerbitController = TextEditingController(text: book?['penerbit'] ?? '');
    final _tahunController = TextEditingController(text: book?['tahun_terbit'] ?? '');
    final _urlGambarController = TextEditingController(text: book?['url_gambar'] ?? '');
  ...
  Future<void> _tambahBuku(String judul, String pengarang, String penerbit, String tahun, String urlGambar) async {
    try {
      final requestBody = {
        'action': 'create',
        'judul': judul,
  ...
  Future<void> _updateBuku(String id, String judul, String pengarang, String penerbit, String tahun, String urlGambar) async {
    try {
      final response = await http.post(
        Uri.parse("http://localhost/Perpustakaan_2301082001/database/buku.php"),
        headers: {'Content-Type': 'application/json'},
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'tanggal_kembali': DateTime.now().add(const Duration(days: 7)).toString().split(' ')[0],
        }),
      );
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text('Pengarang: ${book['pengarang'] ?? ''}'),
                              Text('Penerbit: ${book['penerbit'] ?? ''}'),
                              Text('Tahun: ${book['tahun_terbit'] ?? ''}'),
                              const SizedBox(height: 8),
                              Row(
                                children: [
                                  IconButton(
                                    icon: const Icon(Icons.edit),
                                    onPressed: () => _showFormDialog(book: book),
                                  ),
                                  IconButton(
